...
router.post('/admin/seed', asyncHandler(async (req, res) => {
  ...
  const tools = [
    ...
    { name: 'ChatGenius', slug: 'chatgenius', description: 'Context-aware AI chatbot builder. Deploy intelligent support bots on your website in minutes.', longDescription: 'ChatGenius lets you build and deploy custom AI chatbots trained on your own documentation, FAQs, and knowledge base. Supports live handoff to human agents, multilingual conversations, and integrates with Slack, WhatsApp, and Intercom.', icon: '💬', category: 'agent', badge: 'free', tags: ['chatbot', 'support', 'automation', 'customer-service'], usageCount: 16000, averageRating: 4.6, reviewCount: 112 },
    // ── IMAGE ─────────────────────────────────────────────────────────────
  ...
  success(res, { message: `${tools.length} tools seeded!` });
}));
...
router.post('/:id/save', protect, asyncHandler(async (req, res) => {
  ...
  await user.save();
  success(res, { saved: !saved, savedTools: user.savedTools });
}));
...
router.post('/:id/reviews', protect, asyncHandler(async (req, res) => {
  ...
  if (!rating || !comment) return error(res, 'Rating and comment required', 400);
  const existing = await Review.findOne({ tool: req.params.id, user: req.user._id });
  if (existing) return error(res, 'You already reviewed this tool', 409);
